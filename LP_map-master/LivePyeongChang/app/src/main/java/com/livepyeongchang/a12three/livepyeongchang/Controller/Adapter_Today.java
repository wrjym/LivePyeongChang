...
public class Adapter_Today extends RecyclerView.Adapter<Adapter_Today.ViewHolder>{

    List<Model_Main> tempArr;
    Context context;

    public Adapter_Today(List<Model_Main> tempArr, Context context){
    ...
    private SimpleDateFormat smf = new SimpleDateFormat("HHmm");
    private TimeManager timeManager = new TimeManager();

    @Override
    public void onBindViewHolder(Adapter_Today.ViewHolder holder, final int position) {

        if(tempArr.get(position).getDate().equals(timeManager.addDate(0, true))) {
            ...
            /* 간소화 */
            String timeTemp = tempArr.get(position).getTime();
            String startTime = timeTemp.substring(0,5);
            int startTimeValue = 0;
            int endTimeValue = 0;
            try {
                startTimeValue = Integer.parseInt(startTime.replace(":", ""));
            }catch (Exception e){}

            String endTime = "";
            try {
                endTime = timeTemp.substring(6, 11);
            }catch (Exception e){}

            try {
                endTimeValue = Integer.parseInt(endTime.replace(":", ""));
            }catch (Exception e){}

            long nowTime = System.currentTimeMillis();
            int currentTimeValue = Integer.parseInt(smf.format(new Date(nowTime)));

            if(currentTimeValue >= startTimeValue && currentTimeValue <= endTimeValue){
                holder.iv_status.setImageResource(R.drawable.live);
                holder.ll_frame.setOnClickListener(new View.OnClickListener() {

                    @Override
                    public void onClick(View view) {
                        AlertDialog.Builder ad = new AlertDialog.Builder(context);
                        ad.setIcon(R.drawable.icon_tv);
                        String negative = "Now Watch!";
                        String title = "Streaming Service";
                ...
                holder.ll_frame.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        String message = "It is not time of the match.";
                        if(MainActivity.isKorean) {
                            message = "경기시간이 아닙니다.";
                        }
